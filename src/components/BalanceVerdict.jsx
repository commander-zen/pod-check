          ...
          {ranked.map((p, rank) => {
            ...
            return (
              <div key={originalIndex} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ fontSize: 10, color: "#475569", width: 14, textAlign: "right" }}>{rank + 1}</div>
...
function getVerdict(spread, bracketSpread) {
  if (spread <= 0.8 && bracketSpread <= 1) {
    return { emoji: "⚖️", label: "WELL MATCHED", color: "#5aaa88", advice: "This pod is balanced. Good game ahead." };
  }
  if (spread <= 1.5 || bracketSpread <= 1) {
    return { emoji: "🟡", label: "MINOR GAP", color: "#c4915a", advice: "Slight power difference — playable but worth noting." };
  }
  if (spread <= 2.5) {
    return { emoji: "⚠️", label: "NOTABLE GAP", color: "#c4915a", advice: "Real difference in power levels. Consider a bracket conversation." };
  }
  return { emoji: "🔴", label: "SIGNIFICANT MISMATCH", color: "#c45c6a", advice: "Big power gap. Someone may want to swap decks." };
}
